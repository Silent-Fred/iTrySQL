...
(function() {
  ...
  CodeMirror.defineMIME("text/x-hsql", {
	    name: "sql",
	    client: set("set database sql syntax ora pgs mys mss db2 collation cached memory"),
	    keywords: set(sqlKeywords + "add all any at authorization cascade case check column commit constraint cross default else elseif end escape except exists explain extract for foreign full grant inner intersect left natural outer plan references revoke rollback savepoint unique using "),
	    builtin: set(itrysql_builtin_types + itrysql_builtin_functions + itrysql_builtin_values),
	    atoms: set("false true null unknown"),
	    operatorChars: /^[*+\-%<>!=]/,
	    dateSQL: set("date time timestamp localtime localtimestamp day month year hour minute second interval")
	  });
